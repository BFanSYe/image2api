...
// env：
//
//	IMAGE2API_PROVIDER_GPT  = "real" | "mock"   (默认 mock)
//	IMAGE2API_PROVIDER_GROK = "real" | "mock"   (默认 mock)
//	IMAGE2API_GPT_BASE_URL  = 默认 base url（账号未配置 base_url 时使用）
//	IMAGE2API_GROK_BASE_URL = 默认 base url
//
...
func buildGPT() provider.Provider {
	mode := strings.ToLower(strings.TrimSpace(os.Getenv("IMAGE2API_PROVIDER_GPT")))
	switch mode {
	case "real", "live", "prod":
		return gpt.New(strings.TrimSpace(os.Getenv("IMAGE2API_GPT_BASE_URL")))
	default:
		return mock.New("gpt")
	}
}
...
func buildGrok() provider.Provider {
	mode := strings.ToLower(strings.TrimSpace(os.Getenv("IMAGE2API_PROVIDER_GROK")))
	switch mode {
	case "real", "live", "prod":
		return grok.New(strings.TrimSpace(os.Getenv("IMAGE2API_GROK_BASE_URL")))
	default:
