...
> 主题：**image2api Blue（IKB · International image2api Blue）** · `#002FA7`
...
│       └── (同上)
├── packages/
│   ├── theme/                   # image2api Blue主题：token、tailwind preset、css 变量
...
| UI 基础 | **shadcn/ui** | 组件改主题为image2api Blue |
...
- 高度 64px + safe-area
- 选中态：图标变image2api Blue、上方 4px 圆点指示
...
| 变体 | 视觉 | 用法 |
|------|------|------|
| `primary` | image2api 蓝渐变 + 白字 + glow shadow | 主 CTA：立即生成、确认充值 |
...
- Empty：image2api Blue插画 + 主操作按钮
- Loading：品牌化 loader（image2api Blue呼吸方块）；列表 skeleton 用 `animate-pulse + bg-klein-100/10`
...
- Logo：`logo-mark.svg`（仅图形）+ `logo-full.svg`（图形+文字），image2api Blue单色 / 白色双版本
- 静态文件 CDN 命名：`https://cdn.image2api.example/static/<hash>.<ext>`
