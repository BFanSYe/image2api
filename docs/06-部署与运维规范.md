...
| dev | https://dev.image2api.example | compose / k8s-dev | 隔离库 | push to `develop` 自动 |
| staging | https://stg.image2api.example | k8s-stg | 脱敏快照 | merge to `release/*` 自动 |
| prod | https://image2api.example | k8s-prod / 单机 compose-prod | 真实 | 手动审批 |
...
services:
  mysql:
    image: mysql:8.0
    container_name: image2api-mysql
    restart: always
    command: --default-authentication-plugin=caching_sha2_password
    environment:
      MYSQL_ROOT_PASSWORD: ${MYSQL_ROOT_PASSWORD}
      MYSQL_DATABASE: image2api
      MYSQL_USER: image2api
  ...
  redis:
    ...
    container_name: image2api-redis
    restart: always
  ...
  api:
    image: registry.image2api.example/backend:${TAG}
    container_name: image2api-api
    restart: always
    command: ["/app/api"]
  ...
  admin-api:
    image: registry.image2api.example/backend:${TAG}
    container_name: image2api-admin-api
    restart: always
    command: ["/app/admin"]
    env_file: env/.env.prod
    expose: ["17188"]
  ...
  openai-api:
    image: registry.image2api.example/backend:${TAG}
    container_name: image2api-openai-api
    restart: always
    command: ["/app/openai"]
    env_file: env/.env.prod
    expose: ["17200"]
  ...
  worker:
    image: registry.image2api.example/backend:${TAG}
    container_name: image2api-worker
    restart: always
    command: ["/app/worker"]
    env_file: env/.env.prod
    networks: [image2api-net]
  ...
  user-web:
    image: registry.image2api.example/frontend-user:${TAG}
    container_name: image2api-user-web
    restart: always
    expose: ["80"]
    networks: [image2api-net]

  admin-web:
    image: registry.image2api.example/frontend-admin:${TAG}
    container_name: image2api-admin-web
    restart: always
    expose: ["80"]
    networks: [image2api-net]
  ...
  nginx:
    ...
    container_name: image2api-nginx
    restart: always
    ports:
      - "17080:17080"
      - "17088:17088"
...
COPY . .
RUN pnpm --filter @image2api/user build
...
server {
  ...
  server_name image2api.example;

  ssl_certificate     /etc/nginx/certs/image2api.example.crt;
  ssl_certificate_key /etc/nginx/certs/image2api.example.key;
...
server {
  ...
  server_name openai.image2api.example;
  location /v1/ {
...
- 仅 **goose / atlas** 执行，CI 阶段单独跑：
  ```
  goose -dir migrations mysql "${IMAGE2API_DB_DSN}" up
  ```
- 必须 **向前兼容**：先发 schema 再发代码，保证滚动期间老/新版本共存可用。

### 7.4 灰度

- API 镜像支持读取 `IMAGE2API_GRAY_PCT`，按 `userID hash % 100` 灰度新功能
