...
> 项目代号：**image2api**（image2api Blue AI 生成平台）
...
| 样式 | **Tailwind v4 + shadcn/ui** | - | 主题色：image2api Blue |
...
│   ├── packages/
│   │   ├── ui/              # 共享组件
│   │   ├── theme/           # image2api Blue主题、Tailwind preset
...
| 环境变量 | `SCREAMING_SNAKE_CASE`，业务前缀 | `IMAGE2API_DB_DSN` |
...
| dev | dev.image2api.example | 自动部署 develop | 隔离库 |
| staging | stg.image2api.example | 自动部署 release/* | 脱敏快照 |
| prod | image2api.example | 手动审批 | 真实数据 |
...
```
IMAGE2API_ENV                    # local/dev/staging/prod
IMAGE2API_DB_DSN                 # MySQL DSN
IMAGE2API_REDIS_ADDR
IMAGE2API_REDIS_PASSWORD
IMAGE2API_JWT_SECRET             # ≥ 32 字节随机串
IMAGE2API_JWT_REFRESH_SECRET
IMAGE2API_AES_KEY                # 32 字节，加密账号池密钥
IMAGE2API_OPENAI_BASE            # 可被账号池条目覆盖
IMAGE2API_GROK_BASE
IMAGE2API_ALIPAY_*  / IMAGE2API_WXPAY_*
IMAGE2API_S3_*                   # 生成结果存储
```
...
5. 全站 **接口签名 + 时间戳**（开放 API），`X-Image2API-Sign` + `X-Image2API-Ts`，5 分钟窗口。
...
| `05-前端规范.md` | image2api Blue主题、多端响应式、组件规范 |
